...
console.log(
  `🌍 ENV : connecté à la base ${process.env.DB_NAME} en tant que ${process.env.DB_USER}`
);
...
// ⚠️ Très important : bien mettre les DEUX URL Netlify
const allowedOrigins = [
  'http://localhost:3000',
  'http://localhost:3001',
  'https://gourmet-delight.netlify.app', // avec tiret
  'https://gourmetdelight.netlify.app',  // sans tiret (celle que tu utilises)
].concat(process.env.CORS_ORIGIN ? [process.env.CORS_ORIGIN] : []);

app.use(
  cors({
    origin: (origin, cb) => {
      // autorise aussi les outils sans Origin (Thunder Client, curl, tests)
